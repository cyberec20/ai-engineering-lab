...
function setExecution(resp) {
  ...
  el("agents").textContent = JSON.stringify(resp.agents_created || [], null, 2);
  el("modelsUsed").textContent = JSON.stringify(resp.models_used || [], null, 2);
...
el("send").addEventListener("click", () =>
  send().catch((e) => addMessage("assistant", `ERROR: ${e.message}`))
);
el("message").addEventListener("keydown", (e) => {
  if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) {
    send().catch((err) => addMessage("assistant", `ERROR: ${err.message}`));
  }
});
el("approve").addEventListener("click", () =>
  approve(true).catch((e) => addMessage("assistant", `ERROR: ${e.message}`))
);
el("reject").addEventListener("click", () =>
  approve(false).catch((e) => addMessage("assistant", `ERROR: ${e.message}`))
);

const port = window.location.port || "80";
el("meta").textContent = `Ctrl+Enter para enviar · /chat + /approve · Puerto ${port}`;
